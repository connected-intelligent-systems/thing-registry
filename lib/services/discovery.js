 ...
 */
async function discoverThings (user, accessToken, { session }) {
  ...
  // todo: how to handle colliding ids from other users ?
  // todo: how to directly import things?
  ...
  return results.map((result, index) => ({
    pluginName: pluginSettings[index].pluginName,
    status: result.status,
    ...(result.status === 'fulfilled' && { thingsFound: result.value.length }),
    ...(result.status !== 'fulfilled' && { error: result.reason })
  }))
}
...
async function run (user, accessToken) {
  ...
  try {
    await session.withTransaction(async () => {
      result = await discoverThings(user, accessToken, { session })
    })
  } finally {
