 ...
 */
async function find (thingId, tenantId, customerId, security) {
 ...
 */
async function update (thingId, tenantId, customerId, security, credentials) {
  const securityDefinition = await prisma.securityDefinition.findFirst({
    where: {
      thingId,
      tenantId,
  ...
  return prisma.securityDefinition.update({
    where: {
      tenantId,
      customerId: securityDefinition.customerId,
      thingId,
      name: security,
