...
const { getTypes, getPublicForms } = require('../utils/thing_description')
const { toPublicThing: toExposedThing } = require('../utils/to_exposed_thing')
const {
  sendCreateEvent,
  sendRemoveEvent,
  sendUpdateEvent
...
async function create (description, tenantId, options = {}) {
  ...
  const createdThing = await prisma.thing.create({
    data: {
      ...
      description,
      publicDescription: toExposedThing(description),
      publicForms: {
        createMany: {
