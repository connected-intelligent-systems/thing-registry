...
async function create (description, tenantId, customerId) {
  ...
  const now = new Date()
  augmentThingDescription(description, {
    tenantId,
    customerId,
...
async function findOneOpenApi (id, tenantId, customerId) {
  ...
  const openapi = await tdToOpenAPI(
    removeTypeAnnotationsFromThingDescription(thing.description)
  )
  return openapi.json
}
...
async function assignCustomer (id, tenantId, customerId, newCustomerId) {
  ...
  const now = new Date()
  augmentThingDescription(thing.description, {
    tenantId,
    customerId,
    createdAt: thing.createdAt,
    updatedAt: now.toISOString()
