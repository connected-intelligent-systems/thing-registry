    ...
    await session.withTransaction(async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          models.resource.createAffordance(targets, sub),
...
async function find (accessToken, query) {
  const readableThingIds = await models.access.check({
    resourceType: 'urn:wot:resources:thing',
    scopes: ['GET'],
    token: accessToken.token,
    namesOnly: true
  })

  if (readableThingIds === undefined) {
    return []
  }

  return models.thing.findByIds(readableThingIds, query)
}
