...
async function update (description, accessToken) {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      // use correct owner to generate target, affordances and security definitions
      const { source, owner, username } = await models.thing.update(
        description
      )
...
async function find (accessToken, query, protocol, host, path) {
  ...
  const totalPages = Math.ceil(
    readablePermissions.totalCount[0].count / query.page_size
  )
  ...
  return {
    ...
    things: await models.thing.findByIds(
      readablePermissions.things.map(thing => thing._id) || [],
      {
        protocol,
        host,
        path
