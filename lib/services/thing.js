...
const { getTypes, removeTypeAnnotationsFromThingDescription } = require('../utils/thing_description')
const {
  sendCreateEvent,
  sendRemoveEvent,
  sendUpdateEvent,
...
async function findOneOpenApi (id, tenantId, customerId) {
  ...
  const openapi = await tdToOpenAPI(removeTypeAnnotationsFromThingDescription(thing.description))
  return openapi.json
}
