...
async function init () {
  ...
  await db.collection(collectionName).createIndex('name', { unique: true })
  await loadPlugins()
}
...
function loadPlugin (init, module, info) {
  return {
    type: init.type,
      ...
      try {
        ...
        if (installedPlugins[pluginName] === undefined) {
          ...
          if (validatePluginType(init.type)) {
            const plugin = loadPlugin(init, pluginModule, pluginInfo)
...
async function findOneDiscovery (id) {
  if (
    installedPlugins[id] !== undefined &&
    installedPlugins[id].type === PluginTypes.Discovery
  ) {
    return installedPlugins[id]
  }
}
