...
const ResourceTypes = Object.freeze({
  ...
  Property: 'urn:wot:resources:property',
  Action: 'urn:wot:resources:action',
  Event: 'urn:wot:resources:event'
})

/**
 * Default scopes for resources
 */
const DefaultScopes = Object.freeze(['GET', 'POST', 'PATCH', 'PUT', 'DELETE'])
...
async function create (
  ...
  const response = await fetch(
    `${KeycloakHost}/auth/realms/${KeycloakRealm}/authz/protection/resource_set`,
    {
      headers: {
...
async function createThing (name, user) {
  return create(name, user, ResourceTypes.Thing)
}
 ...
 * @param {string} user - id of the user
 */
async function createProperty (name, user, attributes) {
  return create(name, user, ResourceTypes.Property, attributes)
}
 ...
 * @param {string} user - id of the user
 */
async function createAction (name, user, attributes) {
  return create(name, user, ResourceTypes.Action, attributes)
}
 ...
 * @param {string} user - id of the user
 */
async function createEvent (name, user, attributes) {
  return create(name, user, ResourceTypes.Event, attributes)
}

/**
 * Return the function for a specfic target type
 */
function getCreateFunction (type) {
  switch (type) {
    case 'actions':
      return createAction
    case 'events':
 ...
 * @param {string} user - id of the user
 */
async function createAffordance (targets, user) {
  return Promise.all(
    targets
      .filter(target => target.index === 0)
      .map(target => {
        const createFunc = getCreateFunction(target.type)
        return createFunc(buildNameFromTarget(target), user, {
 ...
 */
async function updateManyProperties (name, user, targets) {
  ...
  return Promise.all([
    ...
    ...targets.map(target => {
      ...
      if (found === undefined) {
        return createAffordance(targets, user)
      }
      return Promise.resolve()
    })
  ])
...
async function deleteResource (id) {
  const response = await fetch(
    `${KeycloakHost}/auth/realms/${KeycloakRealm}/authz/protection/resource_set/${id}`,
    {
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${await getAccessToken()}`
