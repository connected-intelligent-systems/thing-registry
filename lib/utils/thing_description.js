...
function findFormsForAffordanceType (thing, type, { source, tenantId } = {}) {
...
function getAffordanceIds (thing) {
  return [
    ...Object.keys(thing.properties).map(
      key => `${thing.id}/properties/${key}`
    ),
    ...Object.keys(thing.actions).map(key => `${thing.id}/actions/${key}`),
    ...Object.keys(thing.events).map(key => `${thing.id}/events/${key}`)
  ]
}
