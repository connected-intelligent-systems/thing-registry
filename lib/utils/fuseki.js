 ...
 * @type {string}
 */
const FusekiUrl = env.get('FUSEKI_URL').asString()

/**
 * The username for authenticating with the Fuseki server.
 * @type {string}
 */
const FusekiUsername = env.get('FUSEKI_USERNAME').asString()

/**
 * The password for authenticating with the Fuseki server.
 * @type {string}
 */
const FusekiPassword = env.get('FUSEKI_PASSWORD').asString()
...
/**
 * Checks if Fuseki sync is enabled.
 * @returns {boolean} True if Fuseki sync is enabled, false otherwise.
 */
function isSyncEnabled () {
  return FusekiUrl !== undefined
}

/**
 * Creates a dataset with the specified database name.
 * @param {string} dbName - The name of the database for the dataset.
 * @returns {Promise<Response>} - A Promise that resolves to the response from the server.
 * @throws {Error} - If the request to create the dataset fails.
 */
async function createDataset (dbName) {
  const response = await fetch(`${FusekiUrl}/$/datasets`, {
    method: 'POST',
    body: urlencode({
      dbName,
      dbType: 'mem'
...
/**
 * Deletes a dataset from the Fuseki server.
 *
 * @param {string} dbName - The name of the dataset to delete.
 * @returns {Promise<Response>} - A Promise that resolves to the response from the server.
 * @throws {Error} - If the deletion fails, an error is thrown with the corresponding status message.
 */
async function removeDataset (dbName) {
  const response = await fetch(`${FusekiUrl}/$/datasets/${dbName}`, {
    method: 'DELETE',
    headers: {
      Authorization
    }
